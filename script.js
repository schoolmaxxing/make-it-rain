document.addEventListener('DOMContentLoaded', () => {
    ...
    const LESSON_CONTENT = {
        ...
        economy: { title: "Economic Cycles", text: "The economy is versatile and changes according to different factors such as supply and demand, and different business operations. There are different economic cycles of an economy such as: expansion (growth), peak (highest point), contraction (decline), and trough (lowest point)." },
    ...
    const UPKEEP_COSTS = {
        house: 350, // Monthly upkeep per property (taxes, insurance, etc.)
        car: 150    // Monthly upkeep per car (gas, insurance, etc.)
    };

    const QUESTS = [
        // --- Early Game (Foundational) ---
    ...
    const STOCK_PATTERNS = {
        double_top: {
            duration: 7,
            getPriceChange: (step) => {
                if (step < 2) return 0.08; // Rise 1
                if (step < 4) return -0.07; // Fall 1
                if (step < 6) return 0.075; // Rise 2
                if (step === 6) return -0.25; // Breakout down
                return 0;
            }
        },
        double_bottom: {
            duration: 7,
            getPriceChange: (step) => {
                if (step < 2) return -0.08; // Fall 1
                if (step < 4) return 0.07; // Rise 1
                if (step < 6) return -0.075; // Fall 2
                if (step === 6) return 0.25; // Breakout up
                return 0;
            }
        },
        head_and_shoulders: {
            duration: 9,
            getPriceChange: (step) => {
                if (step < 2) return 0.06;  // Left shoulder up
                if (step < 3) return -0.04; // Left shoulder down
                if (step < 5) return 0.10;  // Head up
                if (step < 7) return -0.08; // Head down
                if (step < 8) return 0.05;  // Right shoulder up
                if (step < 9) return -0.04; // Right shoulder down
                if (step === 9) return -0.20; // Breakout down
                return 0;
            }
        },
        cup_and_handle: {
            duration: 9,
             getPriceChange: (step) => {
                if (step === 0) return -0.08; // Cup side 1
                if (step === 1) return -0.05; // Cup bottom
                if (step === 2) return -0.02; 
                if (step === 3) return 0.02;  // Cup side 2
                if (step === 4) return 0.05;
                if (step === 5) return 0.08;
                if (step < 8) return -0.03; // Handle
                if (step === 8) return 0.22;  // Breakout up
                return 0;
            }
        },
        pennant: {
             duration: 5,
             getPriceChange: (step) => {
                if (step === 0) return 0.20;  // Flagpole
                if (step < 4) return (0.02 - (step * 0.01)); // Consolidation
                if (step === 4) return 0.15; // Breakout up
                return 0;
            }
        }
    };
    ...
    // MODIFIED: Logic to handle unattainable Master's quest
    function fastForward(months, newDebt, incomeBoost, newLevel) {
        ...
        // If completing a Doctorate, also complete the Master's quest automatically.
    ...
    // MODIFIED: Economy changes are less frequent
    function updateEconomy() {
        const oldEconomy = gameState.economy;
        const rand = Math.random();
        if (gameState.economy === 'normal') {
            if (rand < 0.03) gameState.economy = 'recession'; // Was 0.05
            else if (rand < 0.08) gameState.economy = 'boom';   // Was 0.15
        } else if (gameState.economy === 'recession' || gameState.economy === 'boom') {
            if (rand < 0.15) gameState.economy = 'normal';   // Was 0.25
        }
